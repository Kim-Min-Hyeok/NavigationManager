...
public struct NavigationManager: View {
    private let initialRoute: String
        ...
        // 배포용 (내부 router 사용)
        public init(initialRoute: String, routes: [RouteEntry]) {
            self.initialRoute = initialRoute
            self.routes = routes
            _router = StateObject(wrappedValue: NavigationRouter())
        }
        
        // 테스트용 (외부 router 주입)
        public init(initialRoute: String, routes: [RouteEntry], router: NavigationRouter) {
            self.initialRoute = initialRoute
            self.routes = routes
        ...
        @ViewBuilder
        private func view(for routeName: String, arguments: AnyHashable? = nil) -> some View {
            if let entry = routes.first(where: { $0.name == routeName }) {
                entry.viewBuilder(arguments)
            } else {
                Text("알 수 없는 경로: \(routeName)")
            }
        }
}
